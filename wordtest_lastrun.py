...
expName = 'wordtest'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'session': '001',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/itsc/Desktop/심리학 조별과제/221113 프로그래밍/Eojeol_NonEojeol_Experiment_trial/wordtest_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text="화면에 제시되는 글자를 보고 어절이면 '/', 비어절이면 'z' 를 최대한 빠르게 눌러주시기 바랍니다.\n\n준비가 완료되었다면, space를 눌러 실험을 시작해 주시기 바랍니다.\n\n먼저 연습시행을 진행합니다. 연습시행에서는 본 시행에 앞서 프로그램을 연습해보고, 정답 및 오답여부를 실시간으로 확인할 수 있습니다.",
    font='Open Sans',
...
practice = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('stimulus_trial.xlsx'),
...
for thisPractice in practice:
    ...
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    text_3.setPos((visualfield, 0))
    text_3.setText(name)
    # keep track of which components have finished
    stimuliComponents = [key_resp_4, text_3]
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if key_resp_4.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_4.getKeys(keyList=['z', 'slash'], waitRelease=False)
    ...
    iti = minDur + (maxDur - minDur) * random()
    text_4.setText('')
    # keep track of which components have finished
    ...
    if not key_resp_4.keys:
        msg_pr = "반응이 입력되지 않았습니다.\n"
    elif key_resp_4.corr or key_resp_4.keys == coranswer:
        msg_pr = "정답입니다!"
    else:
        msg_pr= "오답입니다!\n"
    print(msg_pr, key_resp_4.keys, key_resp_4.corr)
    text_6.setText(msg_pr)
    # keep track of which components have finished
    feedbackComponents = [text_6]
...
_key_resp_3_allKeys = []
# keep track of which components have finished
instructionComponents = [text_7, key_resp_3]
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimulus_new.xlsx'),
